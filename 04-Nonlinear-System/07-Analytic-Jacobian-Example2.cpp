...
//   return
//   end
void fcn07(int* n, double* x, double* fvec, double* fjac, int* ldfjac, int* iflag) {
    ...
    if (*iflag == 1) {
        ...
        fvec[1] = std::pow(x[0], 2) + std::pow(x[1], 2) + std::pow(x[2], 2) - 5;
        fvec[2] = std::exp(x[0]) + x[0] * x[1] + x[0] * x[2] - 1;
    }
    // Jacobi matrix J(x)
    else if (*iflag == 2) {
        fjac[0] = 1.0;                          fjac[3] = 1.0;          fjac[6] = 1.0;
        fjac[1] = 2.0 * x[0];                   fjac[4] = 2.0 * x[1];   fjac[7] = 2.0 * x[2];
        fjac[2] = std::exp(x[0]) + x[1] - x[2];	fjac[5] = x[0];         fjac[8] = -x[0];
    }
}
...
void analyticJacobianExample2()
{
    ...
    std::ofstream ofs;
    ofs.open("roots/roots_07.txt");
    if (!ofs.is_open()) {
        std::cerr << "[Error] cannot open roots_07.txt" << std::endl;
        return;
    }
    ...
    hybrj1_(fcn07, &n, x, fvec, fjac, &ldfjac, &tol, &info, wa, &lwa);
