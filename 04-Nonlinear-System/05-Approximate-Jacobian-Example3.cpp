...
void fcn05(int* n, double* x, double* fvec, int* iflag) {
    // Functions
    // 3y^2 - 6y - x - 3 = 0
    // 2x^3 - 12x - y - 1 = 0
...
void approximateJacobianExample3()
{
    ...
    std::ofstream ofs;
    ofs.open("roots/roots_05.txt");
    if (!ofs.is_open()) {
        std::cerr << "[Error] cannot open roots_05.txt" << std::endl;
        return;
    }
    ...
    for (double i = -4; i <= 4; i += 1.0) {
        for (double j = -5; j <= 5; j += 1.0) {
            ...
            hybrd1_(fcn05, &n, x, fvec, &tol, &info, wa, &lwa);
